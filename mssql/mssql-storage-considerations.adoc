...
Aggregate sind die unterste Speicherebene für AFF-Speicherkonfigurationen. Im Internet findet sich vereinzelt ältere Dokumentation, die empfiehlt, die E/A auf verschiedene Gruppen von Laufwerken zu verteilen. Dies wird mit ONTAP nicht empfohlen. NetApp hat verschiedene Tests zur Charakterisierung der I/O-Last mit gemeinsam genutzten und dedizierten Aggregaten durchgeführt, wobei Datendateien und Transaktionsprotokolldateien getrennt waren. Die Tests zeigen, dass ein großes Aggregat mit mehreren RAID-Gruppen und Laufwerken die Speicherleistung optimiert und verbessert und aus zwei Gründen für Administratoren einfacher zu verwalten ist:

* Ein großes Aggregat/SA macht die E/A-Fähigkeiten aller Laufwerke für alle Dateien verfügbar.
