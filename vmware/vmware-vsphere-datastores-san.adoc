...
[role="lead"]
NetApp ONTAP bietet Block-Storage der Enterprise-Klasse für VMware vSphere unter Verwendung von iSCSI, Fibre Channel Protocol (FCP, kurz FC) und NVMe over Fabrics (NVMe-of). Nachfolgend finden Sie Best Practices zur Implementierung von Blockprotokollen für VM-Storage mit vSphere und ONTAP.
