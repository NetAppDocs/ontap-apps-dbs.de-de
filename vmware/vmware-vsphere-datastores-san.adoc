...
vSphere beinhaltet integrierte Unterstützung für mehrere Pfade zu Speichergeräten. vSphere kann den Typ des Speichergeräts für unterstützte Speichersysteme erkennen und den Multipathing-Stack automatisch so konfigurieren, dass er die Funktionen des verwendeten Speichersystems unterstützt, unabhängig vom verwendeten Protokoll oder davon, ob ASA, AFF, FAS oder softwaredefiniertes ONTAP verwendet wird.
...
NOTE: Siehe link:https://configmax.broadcom.com/guest?vmwareproduct=vSphere&release=vSphere%208.0&categories=2-0["Tool „VMware Konfigurationsmaxima“"^] für die aktuellsten unterstützten Grenzwerte in ESXi.
...
* Für iSCSI- oder NVMe/TCP-Netzwerke verwenden Sie mehrere VMkernel-Netzwerkschnittstellen in verschiedenen nicht gerouteten Netzwerk-Subnetzen mit NIC-Teaming, wenn mehrere virtuelle Switches vorhanden sind. Sie können auch mehrere physische NICs verwenden, die mit mehreren physischen Switches verbunden sind, um Hochverfügbarkeit und erhöhten Durchsatz bereitzustellen. In ONTAP konfigurieren Sie entweder eine Single-Mode-Schnittstellengruppe für Failover mit zwei oder mehr Verbindungen zu zwei oder mehr Switches, oder Sie verwenden LACP oder eine andere Link-Aggregation-Technologie mit Multi-Mode-Schnittstellengruppen, um Hochverfügbarkeit und die Vorteile der Link-Aggregation bereitzustellen. Beachten Sie, dass NVMe/TCP pro VMkernel-Port und pro physischer Schnittstelle aktiviert sein muss und Sie diese so ausrichten müssen, dass NVMe/TCP-VMkernel-Ports Zugriff auf physische NICs mit aktiviertem NVMe/TCP haben. Die folgende Abbildung veranschaulicht die Multipath-Konnektivität mit IP-basierten SAN-Protokollen. image:vsphere_ontap_image2.png["Multipath-Konnektivität"]
