...
Sobald die Storage-Richtlinie erstellt wurde, kann sie verwendet werden, wenn neue VMs bereitgestellt werden, wie in dargestellt link:overview.html#deploy-vm-using-storage-policy["Bereitstellung der VM mithilfe der Storage-Richtlinie"]. Richtlinien zur Nutzung von Performance-Management-Funktionen mit VASA Provider 7.2 finden Sie in <<Performance Management mit ONTAP Tools 9.10 und höher>>.
