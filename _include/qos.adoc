...
Als Referenzbeispiel ist ein älteres HA AFF A800-System mit zwei Knoten in der Lage, bis zu einer Million zufälliger IOPS zu verarbeiten, bevor die Latenz über eine Millisekunde steigt. Sehr wenige einzelne Workloads würden voraussichtlich solche Werte erreichen. Um dieses AFF A800-Systemarray vollständig auszunutzen, müssen mehrere Workloads gehostet werden, und um dies sicher und mit Vorhersagbarkeit zu tun, sind QoS-Steuerungen erforderlich.

Es gibt zwei Arten von Quality of Service (QoS) in ONTAP: IOPS und Bandbreite. QoS-Steuerungen können auf AFF SVMs, AFF Volumes, AFF Dateien sowie AFF/ASA LUNs und Namespaces angewendet werden.
