...
Die Diagramme veranschaulichen die Vorgänge im Normalbetrieb, bei einem Failover und einem Failback. Zur Vereinfachung wird eine replizierte LUN dargestellt. In einer realen SnapMirror active sync-Konfiguration basiert die Replikation auf AFF-Volumes, wobei jedes Volume eine oder mehrere LUNs enthält, aber um die Darstellung zu vereinfachen, wurde die Volume-Ebene entfernt.
