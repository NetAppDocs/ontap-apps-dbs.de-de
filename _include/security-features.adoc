...
Bereits ab ONTAP 9.12.1 sind Snapshots nicht nur schreibgeschützt, sondern können auch vor versehentlichem oder absichtlichem Löschen geschützt werden. Die Funktion heißt manipulationssichere Snapshots. Eine Aufbewahrungsfrist kann festgelegt und über die Snapshot-Richtlinie durchgesetzt werden. Die resultierenden Snapshots können erst gelöscht werden, wenn sie ihr Ablaufdatum erreicht haben. Es gibt keine administrativen oder Support-Center-Überschreibungen.
