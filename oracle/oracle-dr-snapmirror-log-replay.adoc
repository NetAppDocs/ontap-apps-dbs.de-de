...
Die wichtigste Voraussetzung ist die Isolation der Datendateien in einem oder mehreren dedizierten Containern. Dies kann auf verschiedene Weise erfolgen, beispielsweise durch die Platzierung aller Datendateien auf einer einzelnen LUN/einem einzelnen Namespace, innerhalb eines einzelnen AFF NFS-Volumes oder in einer Multi-Volume- oder Multi-LUN/Namespace ONTAP Konsistenzgruppe. Entscheidend ist, dass die Datendateien nicht mit anderen Dateitypen verunreinigt sind. Der Grund dafür ist, sicherzustellen, dass die Replikation der Datendateien vollständig unabhängig von der Replikation anderer Datentypen wie Archivprotokollen erfolgt. Weitere Informationen zu Dateilayouts und wichtige Details zur Sicherstellung, dass das Storage-Layout Snapshot-freundlich ist, finden Sie unter link:oracle-dp-online-backup.html#data-layout["Datenlayout"].
