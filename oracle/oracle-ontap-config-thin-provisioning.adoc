...
Betrachten wir beispielsweise eine Datenbank auf einem einzelnen 250-GB-LUN in einem 1-TB-Volume auf einem AFF-System. Die Erstellung eines Snapshots würde sofort die Reservierung zusätzlicher 250 GB Speicherplatz im Volume zur Folge haben, um sicherzustellen, dass das Volume aus keinem Grund keinen Speicherplatz mehr hat. Die Verwendung von Teilreservierungen ist im Allgemeinen ineffizient, da es äußerst unwahrscheinlich ist, dass jedes Byte im Datenbank-Volume überschrieben werden muss. Es gibt keinen Grund, Speicherplatz für ein Ereignis zu reservieren, das nie eintritt. Kann ein Kunde jedoch die Speichernutzung in einem Speichersystem nicht überwachen und muss absolut sicher sein, dass der Speicherplatz nie ausgeht, sind 100%ige Teilreservierungen erforderlich, um Snapshots zu verwenden.


TIP: Die Volumenabstraktion ist nur auf ASA bei SAN nicht vorhanden, und daher verwendet diese Plattform keine fraktionellen Reserven.
...
Es besteht stets die Möglichkeit, dass sich die Komprimierbarkeit eines Datensatzes ändert, was zu einem erhöhten Speicherverbrauch führen würde. Dieser erhöhte Verbrauch bedeutet, dass die Komprimierung wie bei anderen Formen des Thin Provisioning hinsichtlich Überwachung und, auf einem AFF System, mithilfe von  `volume-autogrow` und  `snapshot-autodelete` verwaltet werden muss.
...
[TIP]
====
*NetApp empfiehlt* die folgenden Reservekonfigurationen mit AFF und FAS-Plattformen:
