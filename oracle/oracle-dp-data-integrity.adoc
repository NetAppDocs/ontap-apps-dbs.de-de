...
Um Daten in solchen Situationen zu schützen, ermöglicht ONTAP die Konfiguration von Volumes, LUNs oder NVMe-Namespaces für einen speziellen Schutz vor NVRAM-Ausfällen. Wird dieser Schutzmechanismus ausgelöst, wechseln die Speicherressourcen in den Zustand NVFAIL. Dies führt zu E/A-Fehlern, die ein Herunterfahren der Anwendung bewirken, damit keine veralteten Daten verwendet werden. Daten sollten nicht verloren gehen, da jeder bestätigte Schreibvorgang im Storage-Array vorhanden sein sollte.

Üblicherweise fährt ein Administrator die Hosts anschließend vollständig herunter, bevor er die LUNs, Namespaces oder NFS-Volumes manuell wieder online schaltet. Obwohl dieser Vorgang etwas Aufwand erfordert, ist er die sicherste Methode, um die Datenintegrität zu gewährleisten. Nicht alle Daten benötigen diesen Schutz, weshalb das Verhalten von NVFAIL granular konfiguriert werden kann.
