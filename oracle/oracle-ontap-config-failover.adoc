...
Bei routinemäßigen Wartungsarbeiten sollte eine Speicherübernahme oder MetroCluster-Umschaltung transparent erfolgen, abgesehen von einer möglichen kurzen Betriebsunterbrechung aufgrund der Änderung der Netzwerkpfade. Da Netzwerke jedoch komplex sein können und Fehler leicht passieren, empfiehlt NetApp dringend, Übernahme- und Umschaltungsvorgänge gründlich zu testen, bevor ein Speichersystem in Produktion geht. Nur so kann sichergestellt werden, dass alle Netzwerkpfade korrekt konfiguriert sind. In einer SAN-Umgebung sollte die Ausgabe des Befehls  `sanlun lun show -p` sorgfältig geprüft werden, um sicherzustellen, dass alle erwarteten primären und sekundären Pfade verfügbar sind.
