...
Eine zusätzliche Mount-Option namens  `nosharecache` ist erforderlich, wenn (a) DNFS aktiviert ist und (b) eine Quell-NFS-Freigabe mehr als einmal auf einem einzelnen Server (c) mit einer verschachtelten NFS-Einbindung eingebunden wird. Diese Konfiguration findet sich hauptsächlich in Umgebungen, die SAP-Anwendungen unterstützen. Beispielsweise könnte eine einzelne NFS-Freigabe auf einem NetApp System ein Verzeichnis unter  `/vol/oracle/base` und ein zweites unter  `/vol/oracle/home` enthalten. Wenn  `/vol/oracle/base` bei  `/oracle` eingebunden ist und  `/vol/oracle/home` bei  `/oracle/home` eingebunden ist, entstehen verschachtelte NFS-Einbindungen, die vom selben Ursprung stammen.

Das Betriebssystem erkennt, dass  `/oracle` und  `/oracle/home` sich auf derselben NFS-Freigabe befinden, was dasselbe Quelldateisystem ist. Das Betriebssystem verwendet dann denselben Gerätehandle für den Zugriff auf die Daten. Dies verbessert die Nutzung des Betriebssystem-Cachings und bestimmter anderer Operationen, beeinträchtigt jedoch DNFS. Wenn DNFS auf eine Datei wie  `spfile` auf  `/oracle/home` zugreifen muss, kann es fälschlicherweise versuchen, den falschen Pfad zu den Daten zu verwenden. Das Ergebnis ist ein fehlgeschlagener E/A-Vorgang. Fügen Sie in diesen Konfigurationen die  `nosharecache` Mount-Option zu jedem NFS-Dateisystem hinzu, das eine Quell-NFS-Freigabe mit einem anderen NFS-Dateisystem auf diesem Host teilt. Dadurch wird das Linux-Betriebssystem gezwungen, einen unabhängigen Gerätehandle für dieses Dateisystem zuzuweisen.
